...
if not keyword_set(pdata) then pdata = 'mvn_pos_mso';'MAVEN_POS_(MARS-MSO)'
if not keyword_set(idata) then idata = 'mvn_B_1sec_MAVEN_MSO ' 
...
get_data,'mvn_pos_mso',data = pos


;get_data,idata,data = imf
get_data,'mvn_B_1sec_MAVEN_MSO',data=imf
get_data,'mvn_pos_mso',data = plot
